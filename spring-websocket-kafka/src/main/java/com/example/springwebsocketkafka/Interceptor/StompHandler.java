package com.example.springwebsocketkafka.Interceptor;
...
public class StompHandler implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        if (StompCommand.CONNECT == accessor.getCommand()) {
            ...
            try {
                producer.sendMessage(ChatMessage.builder().type(ChatMessage.MessageType.ENTER).roomId(roomId).sender(name).build());
            ...
            try {
                producer.sendMessage(ChatMessage.builder().type(ChatMessage.MessageType.QUIT).roomId(roomId).sender(name).build());
            }catch (JsonProcessingException e){
                e.printStackTrace();
            }
